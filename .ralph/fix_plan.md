...
- [x] **P2-AC4** — Run `pytest tests/unit/test_multi_year_carry_forward.py` and assert that `load_prior_year_defaults(user_id, year - 1)` returns a dict with at least `rrsp_room`, `capital_loss_carryforward`, and `foreign_tax_credits` populated from the previous `TaxReturn`; wizard step 3 pre-fills those values without overwriting any field the user has already typed.
...
- [x] P2-AC4 — Multi-year carry-forward: new `services/prior_year.py` with `load_prior_year_defaults(user_id, year)` (queries `TaxReturn` by user+filing_year, reads encrypted `fields`, extracts rrsp_room / capital_loss_carryforward / foreign_tax_credits with alias support for top-level, `wizard_data`, and `user_answers` scopes) + `prefill_wizard_data()` that merges defaults into wizard state without overwriting user-typed values; 10 new tests (zero-default when no prior return, alt field names, multi-user isolation, most-recent-wins, immutable input); 647→657 passing
- [x] P2-AC2 — TaxReturn audit log: new `TaxReturnEvent` model (`tax_return_events` table) + SQLAlchemy `after_insert`/`after_update` mapper hooks; create → row with `before_hash=NULL`, update → distinct sha256 before/after hashes, status change → `event_type="status_change"` (dominates when combined with field edits); identical payloads → identical hashes; no-op flush emits no extra rows; `user_id` NOT NULL enforced at schema; 7 new tests in `tests/unit/db/test_tax_return_events.py`; 657→664 passing
- [x] P2-AC1 — Encrypted TaxReturn.fields migration: new alembic revision `e2f8a9c1b3d4_taxreturn_fields_and_audit_log.py` adds `tax_returns.fields` LargeBinary column + creates `tax_return_events` table + data-migrates any pre-existing plaintext-JSON rows by Fernet-encrypting them in place; also flipped `disable_existing_loggers=False` in `alembic/env.py` so app loggers survive when tests run `alembic upgrade head` mid-suite; 6 new tests verify column exists, raw bytes start with `gAA` Fernet prefix and don't parse as JSON, bulk rows all encrypted, ORM round-trips dict, plaintext data-migration helper re-encrypts; 664→670 passing
